...
export function buildProps(ctx) {
  ...
  const refs={umbrellas:[],binLids:[],crates:[],fans:[],lampGlow:[]};
  function capture(start,pivot,name){const parts=group.children.slice(start),g=new THREE.Group();g.name=name;g.userData.dynamic=true;g.position.fromArray(pivot);group.add(g);group.updateMatrixWorld(true);for(const part of parts)g.attach(part);return g;}
  ...
  const bikeStart=group.children.length;
  ...
  refs.bicycle=capture(bikeStart,[bx,.30,-1.25],'薄荷色自行车');
  ...
  for(let i=0;i<umbrellas.length;i++) {
    ...
    refs.umbrellas.push(capture(start,[xx,.36,zz],'折叠雨伞'));
  }
  ...
  for(let i=0;i<3;i++){
    ...
    const lid=box(.63,.13,.61,4.35,1.16,z,color);lid.userData.dynamic=true;refs.binLids.push(lid);
  ...
  const lampGlow=box(.38,.045,.49,5.22,6.147,3.66,warm,glow(warm,2.2));lampGlow.userData.dynamic=true;lampGlow.material=lampGlow.material.clone();refs.lampGlow.push(lampGlow);
  ...
  for(const xx of [-1.96,-.53,.90]) {
    cyl(.048,.058,.61,xx,.64,3.54,'#b6c8c5');
    sphere(.060,xx,.96,3.54,'#d6dfd0');
  }
  box(2.93,.14,.075,-.53,.79,3.54,'#c5d1c7');
  box(2.93,.036,.086,-.53,.86,3.54,'#dce2d5');
  for(const xx of [-1.91,.85]) box(.10,.085,.015,xx,.79,3.59,'#e9bb68',glow('#e9bb68',.2));
  ...
  for(let i=0;i<2;i++) {
    ...
    const fanStart=group.children.length;
    ...
    refs.fans.push(capture(fanStart,[xx-.17,.75,zz-.286],'空调风扇'));
  ...
  for(let i=0;i<3;i++){
    ...
    const crate=capture(start,[xx,yy-.145,zz],'搬运箱');refs.crates.push({group:crate,position:crate.position.clone()});
  }
  ...
  return {...refs,lampPosition:[5.22,6.06,3.66],vendingPosition:[vx,1.8,vz+.6],vending:{position:new THREE.Vector3(vx,.3,vz),dispensePosition:new THREE.Vector3(vx-.08,.75,vz+.68)}};
}
