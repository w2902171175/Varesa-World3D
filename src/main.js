...
buildGround(ctx);const door=buildShop(ctx,roof),interior=buildInterior(ctx),props=buildProps(ctx);
...
const dynamicGroups=[];staticGroup.traverse(o=>{if(o.isGroup&&o.userData.dynamic)dynamicGroups.push(o);});for(const part of dynamicGroups)batchStatic(part);
...
canvas.addEventListener('dblclick',()=>{if(game.mode==='overview')reset();});
canvas.addEventListener('contextmenu',e=>e.preventDefault());
canvas.addEventListener('keydown',e=>{
 if(game.mode!=='overview')return;
...
addEventListener('resize',()=>{const newAspect=innerWidth/innerHeight;camera.aspect=newAspect;controls.maxDistance=Math.max(65,54/newAspect);if(game.mode==='overview'&&(lastAspect<1)!==(newAspect<1))fitInitial();lastAspect=newAspect;camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight);composer.setSize(innerWidth,innerHeight);});
const game=createGame({scene,camera,canvas,controls,props,interior,door,resetOverview:reset});
...
function animate(now){requestAnimationFrame(animate);if(document.hidden||contextLost){previous=now;return;}
 const dt=Math.min(.05,(now-previous)/1000);previous=now;time+=dt;if(game.mode==='overview'||game.mode==='loading')controls.update();game.update(dt,time);
 // Looking down into the model lifts away the opaque roof without a UI button.
 const elevation=Math.PI/2-controls.getPolarAngle();const targetOpacity=game.mode==='play'?(game.inside?0:1):1-THREE.MathUtils.smoothstep(elevation,.77,1.1);
 ...
 road.material.uniforms.time.value=reduceMotion?0:time;weather(reduceMotion?0:time);composer.render();frameCount++;
